...
const normDate = (raw?: string) => {
    ...
    if (digits.length === 8) return digits; // ddMMyyyy
...
export const SyncfusionEditorModal: React.FC<SyncfusionEditorModalProps> = ({
    ...
    scanInput,
    setScanInput,
    onTestInsertNoiCuTru, // hiện chưa dùng, giữ lại để sau mở rộng
    isProcessingFill, // hiện chưa dùng trực tiếp trong UI, nhưng giữ để tuỳ biến sau
    onChangeTemplate,
    onResetDocument,
    onPrintDocument,
    ...
    // 2 mode:
    // - "ntsoft": nhận dữ liệu qua socket (app mobile NTSoft Document AI)
    // - "scanner": nhập/dán tay chuỗi pipe "cmnd|cccd|..."
    ...
    // Auto-focus textarea khi mở dialog hoặc chuyển qua tab "scanner"
    React.useEffect(() => {
        if (!open) return;
        const t = setTimeout(() => {
    ...
    // sync external index -> internal state
    React.useEffect(() => {
    ...
    // scan placeholder từ blob/url
    React.useEffect(() => {
        if (!open) {
            setPlaceholderState({ loading: false, groups: [], error: undefined });
            updateSelectedIndex('default');
            // allow parent to re-receive summary when dialog opens again
            lastSerializedSummaryRef.current = '';
            return;
        }
    ...
    // đảm bảo selected index nằm trong list index hợp lệ
    React.useEffect(() => {
        if (!placeholderIndexes.length) {
            if (selectedPlaceholderIndex !== 'default') {
                updateSelectedIndex('default');
            }
            return;
        }
    ...
    const parseScannerPayload = (text: string): { canon: Canon | null; error?: string } => {
        ...
        const lines = cleaned
            ...
            .filter(Boolean);
        if (!lines.length) {
            return { canon: null, error: 'Không có dữ liệu để phân tích.' };
        }
        ...
        if (headerIdx >= 0) {
            valuesIdx = headerIdx === 0 ? 1 : headerIdx - 1;
            ...
            defKeys.forEach((k, i) => {
                const v = vals[i];
                if (v == null) return;
                const mapped = ALIAS_TO_CANON[k] as keyof Canon;
                canon[mapped] = v;
            });
    ...
    // dùng cho data nhận qua socket (tab NTSoft)
    const submitFromSocket = React.useCallback(async () => {
        ...
        try {
            if (typeof onHandheldScan === 'function') {
                ...
                // parent sẽ tự quyết định queue/fill
                return;
            }

            const { canon } = parseScannerPayload(s);
            if (canon) {
                await proceedWithFill(toPipe(canon));
            } else {
                setSnackbar({
                    open: true,
    ...
    // dùng cho tab "Scanner" (nhập/dán tay)
                            ...
                            >
                                ...
                                <Box sx={{ mb: 3 }}>
                                    {inputMode === 'scanner' ? (
                                        ...
                                        // Tab "NTSoft": nhận dữ liệu qua socket, không cần nhập tay
                                        <Card variant="outlined" sx={{ p: 2, borderRadius: 1 }}>
                                            ...
                                            >
                                                <Typography variant="body2" color="text.secondary">
                                                    Dữ liệu sẽ tự động chèn từ ứng dụng NTS
                                                    DocumentAI.
                                                </Typography>
                                            </Box>
